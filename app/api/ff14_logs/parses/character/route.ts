...
export async function GET(request: Request) {
  ...
  const path = buildProxyPath(
    `/ff14_logs/parses/character/${encodeURIComponent(characterName)}/${encodeURIComponent(serverName)}/${encodeURIComponent(serverRegion)}`,
    searchParams,
    ["characterName", "serverName", "serverRegion"]
  );

  return proxyFf14Get(path, "获取 FF14 Character Parses 失败");
}
