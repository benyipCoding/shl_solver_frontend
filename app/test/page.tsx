...
interface VerificationErrorDetail {
...
const SHLSolver = () => {
  ...
  const [activeTab, setActiveTab] = useState("solution"); // 'solution' or 'analysis'
  const [selectedLanguage, setSelectedLanguage] = useState("python"); // 'python', 'java', 'javascript'
  const [selectedModel, setSelectedModel] = useState(MODELS[0].id); // Default to Flash

  // Transcription Mode State
  const [isTranscriptionMode, setIsTranscriptionMode] = useState(false);
  const [currentLineIndex, setCurrentLineIndex] = useState(0);

  // --- Solution Verification State (New) ---
  const [verificationImage, setVerificationImage] = useState<string | null>(
    null
  ); // Preview URL
  const [verificationImageData, setVerificationImageData] =
    useState<ImageData | null>(null); // base64 data
  const [verificationLoading, setVerificationLoading] = useState(false);
  const [verificationResult, setVerificationResult] =
    useState<VerificationResult | null>(null); // Error report
  ...
  // --- Verification File Handling (New) ---
  const handleVerificationFileChange = (e: ChangeEvent<HTMLInputElement>) => {
    ...
    if (file && file.type.startsWith("image/")) {
      ...
      setVerificationError("请上传有效的代码屏幕截图 (JPG, PNG)。");
    }
  };
  ...
  const processFiles = async (filesRaw: FileList | File[] | null) => {
  ...
  // --- Solution Verification API Call (New) ---
  const verifyTypedCode = async () => {
    if (!verificationImageData || !result || verificationLoading) return;
    ...
    const systemPrompt = `You are an expert code reviewer and debugger specializing in SHL/HackerRank style online assessments. You are fluent in ${selectedLanguage === "python" ? "Python 3" : selectedLanguage === "java" ? "Java" : "JavaScript (Node.js)"}.
    ...
    const referenceText = `Here is the reference code for comparison:\n\`\`\`${selectedLanguage}\n${getCodeContent()}\n\`\`\``;
      ...
      contents: [
        {
          parts: [
            {
              text: "Verify the typed code in this image against the reference provided below. List every mismatch. Pay close attention to Python indentation.",
            },
            { text: referenceText },
            { inlineData: verificationImageData },
          ],
        },
      ],
    ...
    try {
      ...
      if (
        ...
        const responseText = responseData.candidates[0].content.parts[0].text;
        try {
          setVerificationResult(JSON.parse(responseText));
        } catch (e) {
          console.error("Verification JSON Parse Error:", e);
          throw new Error("比对报告解析失败。");
        }
      } else {
        throw new Error("验证失败，没有收到分析结果。");
      }
    } catch (e: any) {
      console.error(e);
      setVerificationError(`代码比对失败: ${e.message}`);
    } finally {
  ...
  // Helper to render visible spaces for indentation and apply syntax highlighting
  ...
  const renderComplexity = (complexity: string | Complexity) => {
  ...
  if (currentView === "login") {
    return (
      <div className="h-[100dvh] w-full bg-slate-50 flex items-center justify-center p-4 font-sans selection:bg-blue-100 relative overflow-hidden">
        <style>{`
        ...
        <div className="bg-white w-full max-w-md rounded-2xl shadow-xl border border-slate-100 p-6 md:p-8 relative z-10 animate-fadeIn flex flex-col max-h-[95dvh] overflow-y-auto scrollbar-hide">
            ...
            >
              ...
              <div className="space-y-1 relative">
                ...
                <div className="flex space-x-3">
                  ...
                  <div
                    className="w-24 md:w-28 h-[38px] md:h-[42px] bg-slate-100 rounded-xl border border-slate-200 flex items-center justify-center relative overflow-hidden select-none cursor-pointer group hover:border-slate-300 transition-colors"
                    title="点击刷新"
                  >
          ...
          {/* Footer Actions */}
          <div className="mt-4 md:mt-6 text-center border-t border-slate-100 pt-4 md:pt-6 flex-shrink-0">
            <button
              onClick={() => {
                setCurrentView("home");
                setAuthMode("login");
  ...
  return (
    <div className="min-h-screen bg-slate-50 text-slate-900 font-sans selection:bg-blue-100 pb-10 relative">
      <style>{`
        ...
        /* Custom scrollbar for solution tab */
      ...
      {isTranscriptionMode && (
        <div className="fixed inset-0 z-50 bg-slate-900 text-white flex flex-col safe-top animate-fadeIn">
          ...
          <div className="flex-1 flex flex-col justify-center px-4 overflow-hidden relative">
            ...
            <div
              className="opacity-30 text-sm md:text-base font-mono mb-6 ml-2 select-none pointer-events-none truncate"
            ...
            {/* Active Line (Giant) */}
            <div className="bg-[#1e1e1e] p-6 rounded-2xl border-l-4 border-blue-500 shadow-2xl font-mono text-xl md:text-3xl leading-relaxed tracking-wide min-h-[120px] flex items-center text-[#d4d4d4]">
            ...
            <div
              className="opacity-30 text-sm md:text-base font-mono mt-6 ml-2 select-none pointer-events-none truncate"
              dangerouslySetInnerHTML={{
                __html:
              ...
              <div className="bg-white rounded-2xl shadow-sm border border-slate-200 overflow-hidden relative animate-fadeIn">
                ...
                <div className="p-3 md:p-4 bg-white border-t border-slate-100 space-y-3">
                  ...
                  <button
                    ...
                    disabled={loading}
                    className={`w-full py-3 rounded-xl font-bold text-base md:text-lg flex items-center justify-center shadow-md transition-all ${loading ? "bg-slate-100 text-slate-400 cursor-not-allowed" : "bg-gradient-to-r from-blue-600 to-indigo-600 hover:from-blue-700 hover:to-indigo-700 text-white hover:shadow-lg"}`}
                  >
                    {loading ? (
                      <>
          ...
          <div className="flex flex-col h-full min-h-[400px]">
            {result ? (
              <div className="bg-white rounded-2xl shadow-lg border border-slate-200 overflow-hidden flex flex-col h-full animate-fadeIn">
                ...
                {/* Content: Added scrolling class */}
                <div className="flex-1 overflow-y-auto p-4 md:p-6 bg-slate-50 solution-content">
                  {activeTab === "solution" && (
                    <div className="space-y-4 md:space-y-6">
                      ...
                      <div className="relative group">
                        <div className="absolute top-0 left-0 w-full h-full bg-[#1e1e1e] rounded-xl opacity-0 group-hover:opacity-5 transition-opacity pointer-events-none"></div>
                        <pre className="bg-[#1e1e1e] text-[#d4d4d4] p-4 md:p-5 rounded-xl overflow-x-auto text-xs md:text-sm font-mono leading-relaxed border border-slate-800 shadow-inner max-h-[50vh]">
                          <code
                            dangerouslySetInnerHTML={{
                      ...
                      {/* --- Visual Diffing / Verification Panel (New) --- */}
                      <div className="bg-white p-4 md:p-5 rounded-xl border border-slate-200 shadow-sm mt-6 border-l-4 border-l-orange-400">
                        <div className="flex items-center justify-between mb-4">
                          <h4 className="font-semibold text-slate-800 flex items-center text-sm md:text-base">
                            <ScanSearch className="w-5 h-5 mr-2 text-orange-500" />{" "}
                            拍照找茬纠错 (Beta)
                          </h4>
                        ...
                        {!verificationImage ? (
                          <div
                            ...
                            className="border-2 border-dashed border-slate-200 rounded-xl p-6 flex flex-col items-center justify-center text-center hover:border-orange-300 hover:bg-orange-50 transition-all cursor-pointer bg-slate-50"
                          >
                          ...
                          <div className="space-y-4">
                            <div className="relative rounded-lg overflow-hidden border border-slate-200 bg-slate-100 animate-fadeIn">
                              <img
                            ...
                            <button
                              ...
                              className={`w-full py-2.5 rounded-lg font-semibold text-sm flex items-center justify-center transition-all ${verificationLoading ? "bg-slate-100 text-slate-400" : "bg-orange-500 hover:bg-orange-600 text-white shadow"}`}
                            >
                              {verificationLoading ? (
                                <>
                        ...
                        {/* Verification Loading/Results/Error (Absolute positioned relative to panel to avoid jitter) */}
                        <div className="relative min-h-[20px] mt-4">
                          {verificationLoading && (
                            <div className="absolute inset-0 flex items-center justify-center text-orange-500 animate-fadeIn bg-white/90 z-10 rounded">
                          ...
                          {verificationError && (
                            <div className="bg-red-50 text-red-700 p-3 rounded-lg flex items-start border border-red-100 text-xs animate-fadeIn z-10">
                          ...
                          {verificationResult && (
                            <div className="space-y-4 animate-fadeIn">
                              {/* Summary Alert */}
                              <div
                                className={`p-3 rounded-lg border text-xs flex items-start ${verificationResult.has_errors ? "bg-amber-50 text-amber-900 border-amber-100" : "bg-green-50 text-green-900 border-green-100"}`}
                              >
                                {verificationResult.has_errors ? (
                                  <AlertTriangle className="w-4 h-4 mr-2.5 mt-0.5 text-amber-600 flex-shrink-0" />
                                ) : (
                                  <CheckCircle className="w-4 h-4 mr-2.5 mt-0.5 text-green-600 flex-shrink-0" />
                                )}
                                <div>
                              ...
                              {verificationResult.has_errors &&
                                ...
                                verificationResult.errors.length > 0 && (
                                  <div className="space-y-3">
                                    <h5 className="text-xs font-bold text-slate-700 ml-1">
                                      详细错误报告：
                                    </h5>
                                    {verificationResult.errors.map(
                                      (err, idx) => (
                                        <div
                                          key={idx}
                                          className="bg-slate-50 border border-slate-200 rounded-lg p-3 text-xs font-mono relative border-l-2 border-l-red-400"
                                        >
                                          <div className="absolute top-2 right-2 px-1.5 py-0.5 bg-red-100 text-red-700 rounded text-[10px] font-sans font-medium uppercase tracking-wider">
                                          ...
                                          <p className="text-slate-500 mb-2 font-sans">
                                            对应参考代码第{" "}
                                            <span className="font-bold text-slate-700">
                                              {err.reference_line}
                                            </span>{" "}
                                          ...
                                          <div className="grid grid-cols-1 sm:grid-cols-2 gap-x-3 gap-y-2 mb-2 text-[11px]">
                                            <div className="bg-green-50 p-2 rounded border border-green-100 break-all">
                                              <span className="text-green-700 font-sans font-bold">
                                                [应为]
                                              </span>
                                              <br />
                                            ...
                                            <div className="bg-red-50 p-2 rounded border border-red-100 break-all">
                                              <span className="text-red-700 font-sans font-bold">
                                                [实际识别为]
                                              </span>
                                              <br />
                                          ...
                                          <p className="text-amber-800 font-sans bg-amber-50 p-2 rounded border border-amber-100 italic">
                                            👉 修改建议：{err.message}
                                          </p>
                                        </div>
                                      )
              ...
              <div className="h-full flex flex-col items-center justify-center text-slate-400 p-8 border-2 border-dashed border-slate-200 rounded-2xl bg-slate-50 min-h-[300px]">
